...
- For each agent, thought about the environment and output only one action (not a sequence), then wait for the next observation, and repeat this process until all orders are completed.
...
{{
    "reasoning": "...",
    "plan": {{
      "agent1": [{{"action": "MoveTo", "target": [x1, y1]}}],
      "agent2": [{{"action": "Interact", "target": "station_name"}}],
      ...
    }}
}}
```
...
Model Output:
Reasoning: The goal is to make a salad, which requires chopped lettuce and tomato. I have two agents, so I will assign one to each ingredient to work in parallel. Agent1 starts at (1, 4), closer to the lettuce dispenser (dispenser1), so it will handle lettuce. Agent2 starts at (6, 4), closer to the tomato dispenser (dispenser2), so it will handle tomatoes. The first step for both is to move to their assigned ingredient dispensers. The closest empty tiles are (1, 1) for dispenser1 and (2, 1) for dispenser2.
```json
{{
  "agent1": [{{"action": "MoveTo", "target": [1, 1]}}],
  "agent2": [{{"action": "MoveTo", "target": [2, 1]}}]
}}
```
