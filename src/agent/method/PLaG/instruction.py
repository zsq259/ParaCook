...
- You should first construct a graph with the nodes and edges first to represent step ordering constraints, and also construct a dictionary to represent time needed for each step. Use the graph and dictionary to calculate the shortest possible time needed for the task. 
...
Please think step by step and first explain your reasoning process in detail using natural language, including graph construction, task allocation, key operation sequences, collaboration details, and time arrangement. After the reasoning, output the standard JSON format action list without any additional explanations or content.
...
{world_json}
Please think step by step and first analyze the error:
- First check if your graph construction is correct and whether the time dictionary accurately reflects the time needed for each step.
- Then identify which specific action(s) in your previous plan caused the error, and explain why they are invalid based on the rules and constraints.
- Then refine your previous graph and action plan to avoid this error while still following all the rules and constraints.
